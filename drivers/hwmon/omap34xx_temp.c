 ...
 * Mod 1 for Eclair 2.1 by Tasmanian_Droid
 * Mod 2 for Froyo B/W Compat w/ 2.1 by DustinMJ, P3Droid
 *
...
static void omap34xx_update(struct omap34xx_data *data)
{
	...
	if (!data->valid
		...
		/* Dustin Jorge ::: 07/14/10
		   We can wait for this to timeout if needed, either way EOCZ will be up. Return 
		   value shouldn't be checked here, and we should not err here or we neglect to 
		   close SOC bit. After that we just error, so remedy by removing if/err check. */
